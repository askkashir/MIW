      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <View style={styles.streakBadge}>
            ...
            <Text style={[styles.streakText, { color: colors.white }]}>{streak}</Text>
          </View>
          <Pressable 
        ...
        <View style={styles.welcomeSection}>
          <Text style={[styles.title, { color: colors.textPrimary }]}>{getGreeting()}, {firstName}.</Text>
            ...
            return (
              <View key={day} style={styles.dayCol}>
                ...
                {isFlame ? (
                  ...
                  <Text style={[styles.dayNum, { color: colors.textPrimary }]}>{dayNum}</Text>
                )}
              </View>
            );
          })}
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>How would you like to think today?</Text>
          <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>Explore your guided topics.</Text>

          <View style={[styles.actionCardsGroup, { backgroundColor: colors.surface }]}>
            <Pressable
            ...
            >
              ...
              <View style={styles.actionContent}>
                <Text style={[styles.actionCardTitle, { color: colors.textPrimary }]}>{continueModuleTitle}</Text>
                <Text style={[styles.actionCardSubtitle, { color: colors.textSecondary }]}>Last activity: {moduleActivityText}</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} />
            </Pressable>

            <Pressable style={[styles.actionCard, { borderBottomColor: colors.border }]} onPress={() => navigation.navigate('ModulesStack')}>
              <View style={styles.actionIconPlaceholder}>
                <Ionicons name="grid-outline" size={24} color="#D2743E" />
              </View>
              <View style={styles.actionContent}>
                <Text style={[styles.actionCardTitle, { color: colors.textPrimary }]}>Start a New Module</Text>
                <Text style={[styles.actionCardSubtitle, { color: colors.textSecondary }]}>{incompleteModules} modules available</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} />
            </Pressable>

            <Pressable style={[styles.actionCard, { borderBottomColor: colors.border }]} onPress={() => navigation.navigate('JournalStack')}>
              <View style={styles.actionIconPlaceholder}>
                <Ionicons name="pencil" size={24} color="#27AE60" />
              </View>
              <View style={styles.actionContent}>
                <Text style={[styles.actionCardTitle, { color: colors.textPrimary }]}>Free Write</Text>
                <Text style={[styles.actionCardSubtitle, { color: colors.textSecondary }]}>Last Entry: {lastEntryText}</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} />
            </Pressable>

            <Pressable style={styles.exploreBtn} onPress={() => navigation.navigate('ModulesStack')}>
              <Text style={[styles.exploreText, { color: colors.textSecondary }]}>Explore Modules</Text>
        ...
        <View style={styles.section}>
          ...
          <Text style={[styles.sectionSubtitle, { color: colors.textSecondary }]}>Continue your growth journey.</Text>
          <View style={[styles.actionCardsGroup, { backgroundColor: colors.surface }]}>
            {(() => {
              ...
              if (incompleteModules.length === 0) {
                return (
                  <View style={styles.allDoneRow}>
                    ...
                    <Text style={[styles.allDoneText, { color: colors.textPrimary }]}>All modules complete!</Text>
                  </View>
                );
              }
              ...
              return incompleteModules.map((mod) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View style={styles.actionContent}>
                      <Text style={[styles.actionCardTitle, { color: colors.textPrimary }]}>{mod.title}</Text>
                      <View style={styles.modMeta}>
                        <View style={styles.modCategoryPill}>
                          <Text style={styles.modCategoryText}>{mod.category}</Text>
                        </View>
                        <Text style={[styles.actionCardSubtitle, { color: colors.textSecondary }]}>~{mod.estimatedMinutes} min</Text>
                      </View>
                    </View>
