...
export const BottomTabBar: React.FC<BottomTabBarProps> = ({ state, navigation }) => {
  ...
  const renderTab = (tab: TabDef) => {
    ...
    return (
      ...
      >
        ...
        />
        <Text style={[styles.navLabel, { color: isActive ? colors.primaryDark : colors.textSecondary }]}>
          {tab.label}
        </Text>
      </Pressable>
    );
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <Pressable
        style={[styles.floatingBtn, { backgroundColor: colors.primaryDark }]}
        onPress={() => navigation.navigate('JournalStack')}
        accessibilityRole="button"
        accessibilityLabel="Write"
      >
        <Ionicons name="pencil" size={22} color={colors.white} />
        <Text style={[styles.floatingLabel, { color: colors.white }]}>Write</Text>
      </Pressable>
    </View>
  );
};
